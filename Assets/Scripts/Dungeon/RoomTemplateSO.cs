...
public class RoomTemplateSO : ScriptableObject
{
    ...
    [OnOpenAsset(0)]
    static bool OnDoubleClickRoomTemplate(int instanceID)
    {
        RoomTemplateSO roomNodeTemplate = EditorUtility.InstanceIDToObject(instanceID) as RoomTemplateSO;
        if (!roomNodeTemplate) return false;
        PrefabStageUtility.OpenPrefab(AssetDatabase.GetAssetPath(roomNodeTemplate.prefab));
        Selection.activeObject = roomNodeTemplate;
        return true;
    }
